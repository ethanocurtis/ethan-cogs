...
class Minecraft(commands.Cog):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.minecraft_channel: typing.Dict[str, typing.Any] = {
            "servers": {},
            "check_players": False,
            "edit_last_message": False,
            "message_ids": {},  # Store message IDs for each server in each channel
        }
    ...
    async def cog_load(self) -> None:
        ...
        # Start background task for checking servers
        self.bot.loop.create_task(self.check_servers_loop())

    async def cog_unload(self) -> None:
        await self._session.close()
        await super().cog_unload()

    async def check_servers_loop(self) -> None:
        """Background task to check Minecraft servers."""
        await self.bot.wait_until_ready()
        while True:
            await self.check_servers()
            await asyncio.sleep(60)  # Check servers every minute
    ...
    async def check_servers(self) -> None:
        ...
        for channel_id in all_channels:
            ...
            message_ids = all_channels[channel_id].get("message_ids", {})
            for server_url in servers:
                ...
                message_id = message_ids.get(server_url)
                try:
                    if message_id:
                        message = await channel.fetch_message(message_id)
                        await message.edit(embed=embed, attachments=[icon])
                    else:
                        message = await channel.send(embed=embed, file=icon)
                        message_ids[server_url] = message.id
                        await self.config.channel(channel).message_ids.set(message_ids)
                except discord.HTTPException:
                    self.log.error(
                        f"Failed to send/edit message for {server_url} server in {channel.id} channel in {channel.guild.id} guild."
                    )
